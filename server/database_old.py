...
# import base64
# from util import convertB64toBytes, remove_nulls
# import random
# import itertools
...
#         submission["projects"] = remove_nulls([self.project(pcode["pcode"]) for pcode in pcodes if pcode])
#         authors = self.queryAll("SELECT distinct studentEmail FROM Submits where eventId=%s AND year=%s AND code=%s", (eventId, year, code))
#         submission["members"] = remove_nulls([self.student(email["studentEmail"]) for email in authors if email])
...
#         codes = remove_nulls(self.queryAll(query, (email, )))
#         return remove_nulls([self.submission(**kw) for kw in codes])
...
#         pcodes = [i["pcode"] for i in remove_nulls(self.queryAll("SELECT distinct pcode FROM Submits WHERE eventId = %s AND year = %s AND code = %s", (eventId, year, code)))]
#         print(pcodes)
...
#         return remove_nulls([self._processAcc(result) for result in results])
...
#         students = remove_nulls([self.student(email["studentEmail"]) for email in self.queryAll("SELECT distinct studentEmail FROM Submits WHERE eventId = %s and year = %s and code = %s", (eventId, year, sub["code"]))])
...
#         return remove_nulls([self.project(pcode) for pcode in pcodes if pcode])
...
#         return remove_nulls([self.event(**id) for id in ids])
...
#         return remove_nulls([self.project(pcode["pcode"]) for pcode in pcodes])
...
#         institutes = remove_nulls([self.institute(instId) for instId in instIds])
