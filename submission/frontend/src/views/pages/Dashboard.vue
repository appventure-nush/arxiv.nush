<template>
  ...
  <v-timeline side="end" align="start">
    <v-timeline-item dot-color="teal-lighten-3" size="small" v-for="submission in submissions" :key="`${submission.eventId} ${submission.year} ${submission.code}`">
      <div class="d-flex">
        <strong class="me-4"><span v-if="formatDate(submission.start_date) != formatDate(submission.start_date)">{{ formatDate(submission.start_date, false) }} - </span>{{ formatDate(submission.start_date) }}</strong>
        <div>
          <strong>{{ submission.eventId }} {{ submission.year }} - {{ submission.code }}</strong>
...
<v-card height="100%">
  <v-card-title class="pa-8">Best Projects</v-card-title>
      ...
      <v-col cols="12" md="6" lg="4" style="padding-bottom: 0px;">
        ...
        <Chart
          :size="{ width: screenWidth > 960 ? 350 : screenWidth - 80, height: 250 }"
          :data="projStats"
          :margin="margin"
        ...
        <v-card class="pa-8 mx-auto justify-center text-center fill-width" style="position: relative; top: -240px; margin-bottom: 0px;" max-width="1000px">
        <v-card-title style="margin-top: 220px;">Your Projects</v-card-title>

        <v-card-text>
          <i>{{ projStats.map(it => it.count).reduce((partialSum, a) => partialSum + a, 0) < 3 ? 'You should explore more projects!' : "You're excelling at research!" }}</i>
      ...
      <v-col cols="12" md="6" lg="4" style="padding-bottom: 0px;">
        ...
        <v-card class="pa-8 mx-auto justify-center text-center fill-width" style="position: relative; top: -240px" max-width="1000px">
        <v-card-title style="margin-top: 220px;">Your Submissions</v-card-title>

        <v-card-text>
          <i>{{ subStats.map(it => it.count).reduce((partialSum, a) => partialSum + a, 0) < 3 ? 'You should explore more events!' : "Keep going!" }}</i>
...
<v-card-text>
  <i>{{ awardStats.map(it => it.count).reduce((partialSum, a) => partialSum + a, 0) < 2 ? '' : "Good Job!" }}</i>
...
const screenWidth: ComputedRef<number> = computed(
  () => window.innerWidth
);
...
function formatDate(date: Date, year: boolean = true): string {
  return date.toLocaleDateString("en-US", year ? { year: "numeric", month: 'short', day: 'numeric' } : { month: 'short', day: 'numeric' })
}
...
if(appStore.user != null) {
  ...
  loadStrongSubmissions(appStore.userId).then(res => submissions.value = res.filter(it => it.end_date > new Date()).sort((a: StrongSubmission, b: StrongSubmission) => (new Date(a.start_date) - new Date(b.start_date))))

  getStats(appStore.userId).then(
    res => {
