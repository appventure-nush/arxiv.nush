      ...
      <v-col cols="10">
        <v-window v-model="tab">
          ...
          <v-window-item value="option-2">
            ...
            <v-row dense v-if="defaults.prefCardView">
            <v-col :cols="6"  v-for="event in searchEvents()" :key="`${event.eventId}_${event.year}`" style="padding-bottom: 20px;">
              <v-card class="mx-auto pa-4" max-width="500px" height="100%">
                ...
                <vue-markdown class="text-wrap mx-4" :source="(event.about.match(RegExp(/(^.*?[a-z]{2,}[.!?])\s+\W*[A-Z]/)) ?? ['', event.about])[1]" :options="viewOptions"></vue-markdown>
                <v-card-actions>
                  <v-btn
                      color="primary" variant="elevated"
          ...
          >
            <template v-slot:item.eventId="{ item }">
              <a :href="`/events/${item.raw.eventId}/${item.raw.year}`">{{ item.raw.name }}</a>
            </template>
          ...
          <v-row dense v-if="defaults.prefCardView">
            <v-col :cols="6"  v-for="journal in searchAllJournals()" :key="journal.issn" style="padding-bottom: 20px;">
              <v-card class="mx-auto pa-4" max-width="500px" height="100%">
                    <div class="text-overline mx-4 d-flex">
                ...
                <v-list>
                  <v-list-subheader v-if="journal.institutes.length > 0" class="text-overline">Publishers</v-list-subheader>
                  <v-list-item v-for="institute in journal.institutes" :key="institute.instId"
                    active-color="primary">
                    <v-list-item-title>{{institute.name}} ({{institute.instId}})</v-list-item-title>
                    <v-list-item-subtitle>{{ institute.address }}</v-list-item-subtitle>
                  </v-list-item>
                </v-list>
              </v-card>
            </v-col>
...
const eventHeaders = ref([
  ...
  {title: "Competition?", value: "isCompetition", key: "isCompetition", sortable: false},
  {title: "Conference?", value: "isConference", key: "isConference", sortable: false}
])
...
function filterByAdvancedSearch(project: Project): boolean {
  if(deptSearch.value && yearSearch.value) {
    return project.deptId == deptSearch.value && project.year == yearSearch.value;
  }
  if (deptSearch.value) { return project.deptId == deptSearch.value }
  if(yearSearch.value) {return project.year == yearSearch.value}
  return true
}

function searchProjects(): Project[] {
  return search.value.length == 0 ? projects.value.filter(filterByAdvancedSearch) : fuzzysort.go(search.value, projects.value.filter(filterByAdvancedSearch).map(
  (it) => {
    return {
      ...it,
  ...
  }).filter(it => it.score > -3000).map(it => it.obj).slice(0, 6);
}

function searchEvents(): ResearchEvent[] {
  return search.value.length == 0 ? events.value : fuzzysort.go(search.value, events.value, {
    keys: ['name', 'eventId', "year", "about", "format"]
  }).filter(it => it.score > -3000).map(it => it.obj).slice(0, 6);
}

function searchAllJournals(): Journal[] {
  return search.value.length == 0 ? journals.value : fuzzysort.go(search.value, journals.value, {
    keys: ['issn', 'name']
  }).filter(it => it.score > -3000).map(it => it.obj).slice(0, 6);
}
...
onMounted(() => {
  loadOtherProjects(appStore.user?.email ?? "").then(res => projects.value = res)
  loadOtherEvents(appStore.user?.email ?? "").then(res => events.value = res)
