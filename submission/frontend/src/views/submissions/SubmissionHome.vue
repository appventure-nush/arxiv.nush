  ...
  <v-container fluid>
    ...
    <div class="mx-auto pa-4" height="100%">
      <div class="mx-4 d-flex">
        <h1>Your Submissions</h1> <v-spacer />
      ...
      <v-row dense v-if="defaults.prefCardView">
        <v-col :cols="4"  v-for="submission in searchSubmissions()" :key="`${submission.eventId}_${submission.year}_${submission.code}`" style="padding-bottom: 20px;">
      ...
      >
        <template v-slot:item.eventId="{ item }">
          <a :href="`/events/${item.raw.eventId}/${item.raw.year}`">{{ item.raw.eventId }} {{ item.raw.year }}</a>
        </template>
        <template v-slot:item.subTitle="{ item }">
          <a :href="`/submissions/${item.raw.eventId}/${item.raw.year}/${item.raw.code}`">{{ item.raw.subTitle }}</a>
        </template>
...
function searchSubmissions(): SidebarSubmission[] {
  return submissionSearch.value.length == 0 ? allSubmissions.value : fuzzysort.go(submissionSearch.value, allSubmissions.value, {
