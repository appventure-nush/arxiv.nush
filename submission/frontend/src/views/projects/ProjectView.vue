...
<v-container fluid>
        ...
        <v-row>
          <v-col cols="12" class="mx-auto">
            ...
            <!-- </v-flex> -->
            <h1 v-if="!editTitle" class="mx-4 ptitle" @click="if(ownProject) {editTitle = true; tempTitle = project.title}">{{ project.title }}</h1>
            <v-text-field v-else
                ...
                <v-list-subheader class="text-wrap">Project Members</v-list-subheader>
                  ...
                  >
                    ...
                    <template v-slot:append>
                      <v-btn v-if="project.members.length > 1 && ownProject"
                  ...
                  <v-btn block color="primary" @click="newAuthorDialogVisible = true;">ADD NEW</v-btn>
                  </v-list-item>
                  ...
                  <v-list-item
                    ...
                    <v-list-item-subtitle>{{ mentor.jobs.length > 0 ? `${mentor.jobs[0].role}, ${mentor.jobs[0].instId}` : mentor.email }}</v-list-item-subtitle>
                  </v-list-item>
                  ...
                  <v-list-item v-if="ownProject">
                  <v-btn block color="red" @click="deleteEntireProject()">DELETE PROJECT</v-btn>
                  </v-list-item>
            </v-list>
          </v-col>
        ...
        <NewAuthorDialog v-model:visible="newAuthorDialogVisible" :pcode="pcode"></NewAuthorDialog>
...
import { loadProject, loadSubmissions, updateProject, removeStudent, deleteProject } from "@/api/projects"
...
async function deleteEntireProject() {
  if(await deleteProject(pcode.value))
