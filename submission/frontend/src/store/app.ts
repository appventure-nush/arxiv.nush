...
export const useAppStore = defineStore('app', {
  state: () => {
    ...
    const user: Ref<User> = useLocalStorage(
      'user',
      dummyStudent()
    )
    ...
    return {
      user, loggedIn, isStudent, userId
    };
  },
  actions: {
    logout() {
    ...
    async register(username: string, password: string, name: string, isTeacher: boolean, formData: any): Promise<string> {
      const res = isTeacher ? await registerTeacher(username, password, name, formData.deptId) : await registerStudent(username, password, name, formData.gradYear)
      if(res) {
        this.user = res.user ?? dummyStudent();
        return res.message;
      }
