<template>
  <v-app>
      ...
      <v-list dense nav>
        <v-list-item style="text-decoration: none">
          <div align="left" class="pa-2">
            <v-avatar size="80">
              <img :src="`data:image/png;base64,${appStore.user?.pfp}`" />
            </v-avatar>
          </div>
          <v-list-item-title
            ><span style="font-weight: bold" class="text-overline">{{
              appStore.user?.name
    ...
    <v-app-bar app :color="'primary'" dark v-if="appStore.loggedIn">
      ...
      ></v-app-bar-nav-icon>
      <v-toolbar-title> arXiv.nush </v-toolbar-title>
      <v-spacer></v-spacer>
...
const otherRoutes = computed(() => {
  return [
    {
      name: "Profile",
      path: `/${appStore.isStudent ? "students" : "teachers"}/${
        appStore.user == null ? "" : appStore.userId
      }`,
      icon: "mdi-account",
    },
    {
...
const height: ComputedRef<number> = computed(() => window.innerHeight);

const width: ComputedRef<number> = computed(() => window.innerWidth);

onMounted(() => {
  if ("nush_sid" in (appStore.user ?? {})) {
    loadStudent((appStore.user as Student).nush_sid).then(
      (res) => (appStore.user = res ?? appStore.user),
    );
  }
  loadSidebarSubmissions(appStore.userId).then(
    (res) => (hasSubmissions.value = res.length != 0),
  );
});
</script>
