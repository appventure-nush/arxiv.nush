              ...
              <v-col cols="12" md="6">
                <div class="d-flex">
                  <v-text-field
                    ...
                    v-model="code"
                    :rules="codeRules"
                    @update:model-value="computeSubject()"
                    hint="Code allocated to you (e.g. 23.NUSH.018.CS)"
                    required
                  ></v-text-field>
                  <v-text-field
                    label="Year"
                    v-model="year"
                    :rules="yearRules"
                    hint="Year that project is presented."
                    required
                  ></v-text-field>
                  <v-combobox
                    label="Department"
                    ...
                    clearable
                    @update:search="computedSubject = false"
                    v-model="deptSearch"
                  ></v-combobox>
                </div>
              </v-col>
    ...
    codeRules: [
      (value: string | null) => {
        if ((value?.length ?? 0) < 14)
          return "The code should contain at least 14 characters.";
        if ((value?.length ?? 0) > 20)
          return "The code should contain 20 or less characters.";
        return true;
      },
    ],
    yearRules: [
    ...
    titleRules: [
      (value: string | null) => {
        ...
        if ((value?.length ?? 0) > 200)
          return "The title should be shorter than 200 characters.";
        return true;
      },
    ],
    submitLoading: false,
  ...
  methods: {
    async submit() {
      ...
      await createProject(
        this.code,
        this.year,
        this.deptSearch.length == 2 ? this.deptSearch : "RE",
        this.title,
        this.abstract,
        this.teacher.email,
    ...
    computeSubject() {
      const subj = this.code
        .split(".")
        [this.code.replace(/[^.]/g, "").length].toUpperCase();
      if (
        ["BI", "CM", "CS", "EL", "HU", "MA", "PH", "RE"].includes(subj) &&
        (this.computedSubject || this.deptSearch.length == 0)
      ) {
