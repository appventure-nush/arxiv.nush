<template>
    ...
    <v-row>
      <v-col cols="12" sm="2" md="2">
        <v-list class="text-overline" v-model:opened="open">
          ...
          <v-list-group v-for="eventId in getEventIds(events)" :key="eventId" :value="eventId">
          <template v-slot:activator="{ props }">
            <v-list-item class="text-overline text-left"
              v-bind="props"
            >
            <v-list-item-title class="text-overline text-center">{{eventId}}</v-list-item-title>
          </v-list-item>
          </template>
            <v-list-item v-for="year in getYears(events, eventId)"
              :key="year" :value="eventId"
              :href="`/events/${eventId}/${year}`">
              <v-breadcrumbs :items="['', `${year}`]"></v-breadcrumbs>
            </v-list-item>
          </v-list-group>
          <v-list-item v-if="!appStore.isStudent && (appStore.user as Teacher).isAdmin">
            <v-btn color="primary" @click="newEventVisible = true" block>Create</v-btn>
          </v-list-item>
      ...
      <v-col cols="12" sm="6" md="7">
        <div class="mx-4 pa-4">
          <div class="d-flex text-overline">
            <v-breadcrumbs :items="[event.eventId, `${event.year}`]" style="padding-left: 0em; padding-top:0em;"></v-breadcrumbs>
            <v-spacer></v-spacer>{{event.format}} {{ getDefn(event.isCompetition, event.isConference) }}
          </div>
          <h1>{{ event.name }} ({{ event.year }})</h1>
          <p class="text-overline">{{ event.start_date == event.end_date ? event.start_date.toLocaleDateString("en-US", dateOptions) : `${event.start_date.toLocaleDateString("en-US", dateOptions)} - ${event.end_date.toLocaleDateString("en-US", dateOptions)}`  }}</p>

          <br><br>
          <vue-markdown :source="event.about.replaceAll('\n', '<br/>')" :options="viewOptions"></vue-markdown>
      ...
      <v-col cols="12" sm="4" md="3">
        <v-list class="text-left text-wrap">
          <v-list-item v-if="!appStore.isStudent && (appStore.user as Teacher).isAdmin">
            <v-btn color="primary" @click="editEventVisible = true" block>Edit Page</v-btn>
          </v-list-item>
              <v-list-subheader class="text-center text-overline" v-if="event.organisers.length > 0">Organisers</v-list-subheader>
              <v-list-item
                v-for="organiser in event.organisers" :key="organiser.instId"
                :value="organiser.name"
                active-color="primary"
              >
                <v-list-item-title class="text-wrap">{{ organiser.name }}</v-list-item-title>
                <v-list-item-subtitle class="text-wrap">{{ organiser.address }}</v-list-item-subtitle>
              </v-list-item>
              <v-list-subheader class="text-center text-overline" v-if="event.awardTypes.length > 0">Awards Given</v-list-subheader>
              <v-list-item
                v-for="award in event.awardTypes.sort((a, b) => a.length - b.length)" :key="award"
                :value="award" variant="tonal"
                active-color="primary"
              >
                <v-list-item-title class="text-wrap">{{ award }}</v-list-item-title>
                <!-- <v-list-item-subtitle class="text-wrap">{{ organiser.address }}</v-list-item-subtitle> -->
              </v-list-item>
              <v-list-item v-if="(event?.confDoi ??'').length > 0" :href="`https://doi.org/${event.confDoi}`">
                <v-list-item-title class="text-wrap">Visit at {{ event.confDoi }}</v-list-item-title>
              </v-list-item>
          <v-list-item v-if="appStore.isStudent">
            <v-btn color="primary" @click="submitVisible = true" block>Submit my Project</v-btn>
          </v-list-item>
            </v-list>
      </v-col>
      <v-col cols="12">
        ...
        <div class="mx-auto pa-4" height="100%">
          ...
          <v-row dense v-if="defaults.prefCardView">
            <v-col :cols="4"  v-for="submission in searchSubmissions()" :key="submission.code" style="padding-bottom: 20px;">
          ...
          <template v-slot:item.code="{ item }">
            <a :href="`/submissions/${eventId}/${year}/${item.raw.code}`">{{ item.raw.code }}</a>
          </template>
            <template v-slot:item.title="{ item }">
              <a :href="`/submissions/${eventId}/${year}/${item.raw.code}`">{{ item.raw.title }}</a>
            </template>
  ...
  <EditEventDialog v-model:visible="editEventVisible" v-model:event="event"></EditEventDialog>
  <SubmissionDialog v-model:visible="submitVisible" v-model:event="event"></SubmissionDialog>
</template>
...
const headers = ref([
  {title: "Code", value: "code", key: "code", sortable: true, align: "center"},
  {title: "Title", value: "title", key: "title", sortable: true},
  {title: "Project Code", value: "pcode", key: "pcode", sortable: true, align: "center"},
...
function searchSubmissions(): WeakSubmission[] {
  return search.value.length == 0 ? event.value.submissions.filter(it => it.pcode != '') : fuzzysort.go(search.value, event.value.submissions.filter(it => it.pcode != '').map(
  (it: WeakSubmission) => {
    return {
      ...it,
...
const updatePage = (dialog: boolean, prevDialog: boolean) => {
  if(!dialog && prevDialog) {
    loadEvent(eventId.value, year.value).then(res => event.value = res ?? event.value)
