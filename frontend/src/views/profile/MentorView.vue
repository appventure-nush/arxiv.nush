      ...
      <v-col cols="3">
        <v-card class="mx-auto pa-4 text-center">
          <v-avatar size="200px" class="justify-center">
            ...
            <v-card-title class="justify-center text-wrap">{{ mentor.name }}</v-card-title>

            <v-list class="text-left">
              <v-list-subheader v-if="mentor.jobs.length > 0">Jobs</v-list-subheader>
              <v-list-item
                v-for="job in mentor.jobs" :key="job.instId"
                :value="job.role"
                active-color="primary"
              >
                <v-list-item-title>{{ job.role }}, {{ job.dept }}</v-list-item-title>
                <v-list-item-subtitle>{{ job.name }} ({{ job.instId }})</v-list-item-subtitle>
              </v-list-item>
              <v-list-subheader v-if="students.length > 0">Student Collaborators</v-list-subheader>
              <v-list-item
...
import { Student, dummyStudent, ExternalStudent, ResearchMentor, dummyMentor } from '@/types/user'
import { Project } from "@/types/project"
import { useRoute } from 'vue-router'
import {loadStudent, loadCoauthors, updateStudent, loadMentor, loadMentorStudents} from '@/api/user'
