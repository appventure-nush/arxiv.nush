      ...
      <v-col cols="3">
        <v-card class="mx-auto pa-4 text-center">
          ...
          <div class="mx-4 d-flex text-overline">
            {{ isNush ? (teacher as Teacher).deptId : "External" }}
            <v-spacer />
            {{
              isNush
                ? (teacher as Teacher).isAdmin
                  ? "Admin"
                  : ""
                : (teacher as ExternalTeacher).schId
            }}
          </div>
          <!-- </v-flex> -->
          <FileSelector
          ...
          <v-avatar v-else size="200px" class="justify-center">
            <v-img
              ...
              :width="250"
              :src="
                isNush ? `data:image/png;base64,${teacher.pfp}` : default_image
              "
              class="justify-center"
              cover
              alt="Click to select avatar"
...
onMounted(() => {
  ...
  loadTeacher(
    tid.value.includes("@") ? tid.value : `${tid.value}@nushigh.edu.sg`,
  ).then((res) => {
    teacher.value = res;
  });
  loadProjects(tid.value).then((res) => (projects.value = res));

  loadTeacherStudents(tid.value).then((res) => {
    students.value = res;
  });
});
</script>
