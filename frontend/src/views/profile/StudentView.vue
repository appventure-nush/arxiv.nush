        ...
        <v-card class="mx-auto pa-4 text-center">
            <!-- <v-flex class="text-overline"> -->
                <div class="mx-4 d-flex text-overline">
                    {{ isNush ? (student as Student).nush_sid : 'External' }} <v-spacer /> {{ isNush ? `Y${6 + (new Date()).getFullYear() - (student as Student).gradYear ?? 0}` : (student as ExternalStudent).schId }}
                </div>
            <!-- </v-flex> -->
            <FileSelector
                v-if="isOwn && isNush"
          ...
          <v-avatar v-else size="200px" class="justify-center">
              <v-img
                :aspect-ratio="1"
                :width="250"
                :src="isNush ? `data:image/png;base64,${(student as Student).pfp}` : default_image"
                class="justify-center"
                cover
                alt="Click to select avatar"
              ></v-img>
            ...
            <v-card-title class="justify-center text-wrap">{{ student.name }}</v-card-title>

            <v-card-text v-if="!editAbout && isNush" class="justify-center" @click="if(isOwn) {tempAbout = student.about; editAbout = true;}">
              {{  student.about ? student.about : "No About Inserted" }}
            </v-card-text>
            <v-text-field v-if="editAbout && isNush" clearable v-model="tempAbout" v-on:keyup.enter="editAbout = false; updateAbout()"></v-text-field>

            <v-list class="text-left">
              <v-list-subheader>{{isNush ? 'Common Collaborators' : 'Teacher Contact'}}</v-list-subheader>
              <v-list-item
              ...
              <v-list-item v-if="!isNush"
                :prepend-avatar="default_image"
                :href="`/teachers/${(student as ExternalStudent).emergencyEmail}`"
              >
                <v-list-item-title>{{ (student as ExternalStudent).teacherName }}</v-list-item-title>
                <v-list-item-subtitle class="text-wrap">{{ (student as ExternalStudent).emergencyEmail }}</v-list-item-subtitle>
            </v-list-item>
            </v-list>
