<template>
  <v-container fluid class="pa-4 ma-2">
    ...
    <v-col cols="12" sm="6">
      ...
      >
      <v-form
        :style="changePassword ? { display: 'block' } : { display: 'none' }"
        fast-fail
        @submit.prevent
      >
        ...
        <v-text-field
          ...
          :rules="[rules.required, rules.lowerBound, rules.upperBound]"
          clearable
        ...
        <div class="d-flex mt-2">
          ...
          <v-spacer></v-spacer>
          <!-- This adds a gap for the buttons -->
          <v-btn color="red" @click="changePassword = !changePassword"
            >Cancel</v-btn
          >
          <v-btn
            color="primary"
    ...
    <v-dialog v-model="isChangePasswordError">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer
          ><v-btn color="primary" @click="isChangePasswordError = false"
            >Close</v-btn
          >
        </v-card-actions>
      </v-card>
...
async function updatePassword() {
  ...
  const response = await updateDBPassword(
    appStore.user.email,
    oldPassword.value,
    newPassword.value,
  );
