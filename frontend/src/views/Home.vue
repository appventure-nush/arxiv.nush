<template>
  ...
  >
    <section
      ...
      justify="center"
      :height="height"
      style="fill-height: 100%; margin-top: 30vh"
    >
      <v-row align="center" justify="center">
        <v-col align="center" justify="center">
          <span style="font-size: 3em" color="white"> arXiv.nush </span>
    ...
    <section v-scroll-reveal.reset>
      <v-row align="center" justify="center">
        <v-col align="center" justify="center">
          <span style="font-size: 3em"> Doing an ARP at NUS High? </span>
    ...
    <section v-scroll-reveal.reset>
      <v-row align="center" justify="center">
        <v-col align="center" justify="center">
          <span style="font-size: 3em"> Enjoy more comforts! </span>
          <span style="font-size: 1.5em">
    ...
    <section v-scroll-reveal.reset ref="authSection">
      <v-row align="center" justify="center">
        <v-col align="center" justify="center" cols="10">
          <span style="font-size: 3em"> Ready to get started? </span
          ><br /><br />
                  ...
                  <v-row align="center" justify="center" class="ma-4">
                    <v-col cols="12" sm="12" md="6">
                      ...
                      <v-text-field
                        ...
                        :rules="[
                          rules.required,
                          rules.nameLowerBound,
                          rules.nameUpperBound,
                        ]"
                        required
                      >
                      </v-text-field>
                    ...
                    <v-col cols="12" sm="12" md="6">
                      ...
                      <v-text-field
                        ...
                        :rules="[
                          rules.required,
                          passwordRules.lowerBound,
                          passwordRules.upperBound,
                        ]"
                        required
                      >
                      </v-text-field>
                    ...
                    <v-col cols="12" sm="12" md="6" v-else>
                      ...
                      <v-text-field
                        label="Year"
                        v-model="gradYear"
                        :rules="[(it: string) => rules.year(parseInt(it))]"
                        required
                      ></v-text-field>
                    </v-col>
                  </v-row>
...
async function signIn() {
  errorText.value = await appStore.login(
    `${email.value}@nushigh.edu.sg`,
    password.value,
  );
...
async function signUp() {
  ...
  errorText.value = await appStore.register(
    `${email.value}@nushigh.edu.sg`,
    password.value,
    name.value,
    isTeacher.value,
    isTeacher.value
      ? { deptId: deptSearch.value }
...
const rules = {
  ...
  year(value: number | null) {
    if ((value ?? 0) < 2006)
      return "The project should have been presented after 2005.";
    if ((value ?? 0) > 2099)
      return "The project should have been presented in the 21st Century.";
  },
  nameLowerBound: (v: string) =>
    v.length >= 3 || "Name must be at least 3 characters.",
  nameUpperBound: (v: string) =>
    v.length <= 100 || "name must be at most 100 characters.",
};

const passwordRules = {
  lowerBound: (v: string) =>
    v.length >= 8 || "Password must be at least 8 characters.", // if the field is shorter than 8 characters, the error message
  upperBound: (v: string) =>
    v.length <= 20 || "Password must be at max 20 characters.", // if the field is longer than 20 characters, the error message
};
