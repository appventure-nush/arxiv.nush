...
export async function updateProject(
  ...
  await axios
    .post(`${API_URL}/update/project`, { pcode, title, abstract, reportPdf })
    .then((res) => {
      const data = res.data;
...
export async function deleteProject(pcode: string): Promise<boolean> {
  return await axios
    .post(`${API_URL}/delete/project`, { pcode })
...
export async function createSubmission(
  ...
  return await axios
    .post(`${API_URL}/create/submission`, {
      eventId,
      year,
      code,
      subTitle,
    ...
    .then((res) => res.data as StrongSubmission);
}
...
export async function getSubmission(
  ...
  return await parseSubmission(
    await (
      await fetch(`${API_URL}/submission/${eventId}/${year}/${code}`)
    ).json(),
  );
}
...
export async function updateSubmission(
  ...
  subTitle: string,
  subAbstract: string,
): Promise<StrongSubmission | null> {
  // update
  let sub: StrongSubmission | null = null;
  await axios
    .post(`${API_URL}/update/submission`, {
      eventId,
      year,
      code,
      subTitle,
...
export async function loadStrongSubmissions(
  id: string,
): Promise<StrongSubmission[]> {
  return await (
    await (await fetch(`${API_URL}/submissions/all/${id}`)).json()
  ).map(parseSubmission);
}

export async function removeStudentFromSubmission(
  email: string,
  eventId: string,
  year: number,
  code: string,
  ...
  await axios
    .post(`${API_URL}/submission/remove`, { email, eventId, year, code })
    .then((res) => {
      const data = res.data;
      if ("result" in data && data.result == false) return;
...
export async function deleteSubmission(
  ...
  return await axios
    .post(`${API_URL}/delete/submission`, { eventId, year, code })
    .then((res) => res.data.result);
}

export async function addStudentsToSubmission(
  students: String[],
  eventId: string,
  year: number,
  code: string,
) {
  await axios.post(`${API_URL}/submissions/add`, {
    students,
    eventId,
    year,
    code,
