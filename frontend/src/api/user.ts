...
import { User, Student, Coauthor, Teacher, ExternalStudent, ResearchMentor } from "@/types/user"
...
export async function updateStudent(sid: string, about: string, pfp: string): Promise<Student | null> {
  // update
  let student: Student | null = null;
  await axios.post(`${API_URL}/update/student`, { sid, about, pfp }).then(res => {
    const data = res.data;
...
export async function createExternalStudent(email: string, name: string, teacherEmail: string, teacherName: string, instId: string) {
  await axios.post(`${API_URL}/addExtStudent`, { email, name, teacherEmail, teacherName, instId })
}
...
export async function loadTeacherStudents(tid: string): Promise<Array<Student|ExternalStudent>> {
  if(tid.includes("@")) {
    return await (await fetch(`${API_URL}/extTeacherStudents/${tid}`)).json()
  } else {
    return await (await fetch(`${API_URL}/bestStudents/${tid}@nushigh.edu.sg`)).json()
  }
}
...
export async function loadMentorStudents(mid: string): Promise<Array<Student|ExternalStudent>> {
