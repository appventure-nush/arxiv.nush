...
export async function loadEvent(
  ...
  await (await fetch(`${API_URL}/event/${eventId}/${year}`))
    .json()
    .then((res) => {
...
export async function loadEvents(): Promise<Event[]> {
  return await (
    await (await fetch(`${API_URL}/events`)).json()
  ).map((it: any) => parseEvent(it) ?? it);
}
...
export async function createEvent(
  ...
  return await axios
    .post(`${API_URL}/create/researchEvent`, {
      eventId,
      year,
      name,
      start_date,
...
export async function updateEvent(
  ...
  return await axios
    .post(`${API_URL}/update/researchEvent`, {
      eventId,
      year,
      name,
      start_date,
